    ...
    def add_new_tab(self, qurl=None):
        ...
        if not isinstance(qurl, QUrl):
            if isinstance(qurl, str):
                ...
                # 2. Join it properly with the filename
                path = os.path.join(base_path, "landing.html")
                
                if os.path.exists(path):
                    qurl = QUrl.fromLocalFile(path)
                else:
                    # If it still fails, it's likely a permissions issue or 
                    # the file is missing from that specific folder
